...
import { defaultProfileData, defaultSections, defaultThemes } from "@/data/default-data"
...
export default function Home() {
  ...
  const guideSteps = [
    ...
    },
    {
      ...
      content: "Enter your GitHub username and click 'Fetch Data' to automatically import your GitHub stats.",
      position: "right",
    },
    {
      target: "#skills-tab",
      content: "Add your technical skills and technologies you're proficient in.",
      position: "bottom",
    },
    {
      target: "#template-select",
      content: "Choose from 6 professionally designed templates to match your style.",
      position: "left",
    },
    {
      target: "#preview-tab",
      content: "Preview how your GitHub profile will look and copy the markdown when you're done.",
      position: "bottom",
    },
  ]
    ...
    const loadProfileFromStorage = () => {
      ...
      try {
        ...
        if (savedProfile) {
          ...
          trackEvent("profile_loaded", "storage_action", "load_from_localstorage")

          toast({
            title: "Profile loaded",
            description: "Your saved profile has been loaded from browser storage",
          })
        }
      } catch (error) {
        ...
        toast({
          ...
          description: "Could not load your saved profile. Using default profile instead.",
          variant: "destructive",
        })
      } finally {
  ...
  const handleToggleSection = (sectionId) => {
    ...
    if (sectionId === "stats") {
      ...
      setProfileData({ ...profileData, showProjects: !profileData.showProjects })
    } else if (sectionId === "blog") {
      setProfileData({ ...profileData, showBlog: !profileData.showBlog })
    } else if (sectionId === "timeline") {
      setProfileData({ ...profileData, showTimeline: !profileData.showTimeline })
    } else if (sectionId === "education") {
      setProfileData({ ...profileData, showEducation: !profileData.showEducation })
    } else if (sectionId === "contact") {
      setProfileData({ ...profileData, showContact: !profileData.showContact })
    }

    // Track section toggle
    const newState = !sections.find((section) => section.id === sectionId).visible
    trackEvent(newState ? "section_enabled" : "section_disabled", "user_preference", sectionId)
  }
  ...
  const handleFetchGitHubData = async (username) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Found user ${updatedData.name || username} with ${updatedData.publicRepos || 0} repositories`,
      })
    } catch (error) {
      console.error("Error fetching GitHub data:", error)

      // Track error in fetching GitHub data
      trackEvent("github_data_error", "api_action", error.message || "unknown_error")

      toast({
        title: "Error fetching data",
        description: error.message || "Could not fetch GitHub data. Please try again.",
        variant: "destructive",
      })
    } finally {
      setIsFetching(false)
  ...
  const toggleExpandPreview = () => {
    ...
    trackEvent(newState ? "preview_expanded" : "preview_collapsed", "user_action", "preview_toggle")
  }
    ...
    <TooltipProvider>
      <main className="min-h-screen bg-gray-50 dark:bg-gray-950">
        ...
        <div className="container mx-auto px-4 py-8 max-w-7xl">
          ...
          >
            <h1 className="text-4xl font-extrabold tracking-tight shimmer-effect">GitHub Profile README Builder</h1>
            <p className="text-lg text-gray-600 dark:text-gray-400 mt-2">
              Create an impressive GitHub profile page with our easy-to-use builder
            </p>
          </motion.div>
              ...
              <Alert className="bg-primary/10 border-primary/20">
                <div className="flex justify-between items-center">
                  <div className="flex items-center">
                    ...
                    <div>
                      <AlertTitle>Profile loaded from browser storage</AlertTitle>
                      <AlertDescription>
                        Your previously saved profile has been loaded. Any changes will be automatically saved.
                      </AlertDescription>
                    </div>
                  </div>
                  <Button
                    ...
                    onClick={() => {
                      ...
                      trackEvent("reset_button_clicked", "user_action", "reset_profile")
                    }}
                    className="ml-4"
                  >
                    Reset to Default
              ...
              <Alert variant="destructive">
                ...
                <div className="flex justify-between items-center w-full">
                  <div>
                    ...
                    <AlertDescription>This will delete your saved profile and cannot be undone.</AlertDescription>
                  </div>
                  <div className="flex gap-2 ml-4">
                    <Button variant="destructive" size="sm" onClick={handleResetProfile}>
                      Reset
                    </Button>
                    <Button
                      variant="outline"
                      ...
                      onClick={() => {
                        setShowResetAlert(false)
                        trackEvent("reset_cancelled", "user_action", "cancel_reset")
                      }}
                    >
                      Cancel
                    </Button>
          ...
          {isLoadingFromStorage ? (
            <div className="flex justify-center items-center py-20">
              <div className="text-center">
                <RefreshCw size={40} className="mx-auto animate-spin text-primary mb-4" />
                <p className="text-lg font-medium">Loading your profile...</p>
                <p className="text-sm text-gray-500 dark:text-gray-400">Retrieving your saved data</p>
              </div>
            </div>
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              >
                <Card
                  className={`flex-grow shadow-md hover:shadow-lg transition-shadow ${expandedPreview ? "fixed inset-4 z-50 overflow-auto" : ""}`}
                >
                  <CardContent className="pt-6">
                    ...
                    <Tabs defaultValue="preview" className="w-full" onValueChange={handleTabChange}>
                      <TabsList className="grid w-full grid-cols-3">
                        ...
                        <TabsTrigger value="contributions">Contribution Graph</TabsTrigger>
                      </TabsList>
                      <TabsContent
                        value="preview"
                        className="border rounded-md p-4 mt-4 min-h-[500px] bg-white dark:bg-gray-950"
                      >
                        <ProfilePreview profileData={profileData} template={activeTemplate} />
                      </TabsContent>
                        ...
                        {profileData.github ? (
                          <div className="space-y-4">
                            <div className="bg-white dark:bg-gray-800 border rounded-md p-4">
                              <h3 className="text-lg font-semibold mb-4">GitHub Contribution Graph</h3>
                            ...
                            <div className="text-sm text-gray-500 dark:text-gray-400 text-center">
                              <p>This contribution graph is automatically generated based on your GitHub username.</p>
                              <p>
                                It will be included in your README when you enable the "Contribution Graph" option in
                                Settings.
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="text-center py-8 bg-gray-50 dark:bg-gray-800 rounded-lg">
                            <p className="text-gray-500 dark:text-gray-400">
                              Please enter your GitHub username in the Profile tab
                            </p>
                            <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">
                              The contribution graph will be generated automatically
                            </p>
                          </div>
                        )}
                      </TabsContent>
